...
var _url = {
    ...
    CloseDashboard: "DashBoard/CloseDashboard",
...
function DeleteDashboard(dashboardId) {
    ...
    if (current_dashBoardId != dashboardId) {
        ...
        OnDashboardError(client.main.messages.label_delDashBoard);
...
/*************************************/
function CloseDashboard(sender, dashboardId) {
...
function SaveDashboard(sender, dashboardId) {
    ...
    if (enableSave) {
        ...
        clientPostJSON(
            ...
            function (jsonString) {
                ...
                if (result.hasOwnProperty('success')) {
                    ...
                    OnDashboardError(client.main.messages.label_error_dash_save);
        ...
        OnDashboardError(client.main.messages.label_invalidWidgets);
...
function splitDiv(divSplit, selVal) {
    ...
    var rowNum = $("#"+divSplit);
    var divRow = $("#" + divSplit).parents(".divRow");
    var selValue = $(selVal).val();
    var id_row = $(divRow).attr('id');
    id_row = (id_row != undefined) ?
        id_row = id_row.substring(4) : -1;

    var data = {
        id: id_row,
        splitted: (selValue == "50%") ? true : false
    };

    clientPostJSON(
    _url.UpdateRow, clientParseObjectToJson(data),
        function (jsonString) {
            if (data.splitted) {
...
function selectWidget(button) {
    ...
    $(div_select).attr("title", client.main.messages.label_widgetSelection);
    $(div_select).html("<select class='selWdg'><option value='text'>" + client.main.messages.label_wdgText + "</option><option value='chart'>" + client.main.messages.label_wdgChart + "</option><option value='table'>" + client.main.messages.label_wdgTable + "</option></select>");

    options.buttons[client.main.messages.label_ok] = function () {

        var wdg_type = $(div_select).find(".selWdg").val();
            ...
            $(button).parent(".divTd").append("<div class='confWdg' ><input type='hidden' value='" + wdg_type + "' class='valConf'/>" +
                                                "<div class='wdgName'><i class='icon-" + aClass + "'></i>" + wdg_type + " widget</div>" +
...
function ConfigWidget(div) {
    ...
    //$(configWidget).find('.html_editor').jqte();
    ...
    if ($(divWidget).children(".valConf").val() == "text") {
        ...
        //FILL THE TEXT FIELD WITH THE HIDDEN VALUE BY SELECTED LANGUAGE
        ...
        //$(thisContent).jqte({ change: function () { alert("The editor is changed"); } });
        $(thisContent).keyup(
            function () {
                insertLangField($(thisContent), $(divWidget).children(".valConf").val(), divWidget);
            });
        ...
        clientPostJSON(
            ...
            function (jsonString) {
                ...
                /************************/
                if (!_sett_obj.hasOwnProperty("error") && _sett_obj.hasOwnProperty('endpoints')) {
                    ...
                    if ($(divWidget).find("input[name='endPointSource']").val() == "") { $(divWidget).find("input[name='endPointSource']").val(conf.EndPointSource); }
                    if ($(divWidget).find("input[name='endPointDecimalCulture']").val() == "") { $(divWidget).find("input[name='endPointDecimalCulture']").val(conf.DecimalSeparator); }
                    ...
                    $(webServiceSelector).change(function () {
                        ...
                        $(configWidget).find("input[name='endPointSource']").val(conf.EndPointSource);
                        $(configWidget).find("input[name='endPointDecimalCulture']").val(conf.DecimalSeparator);
                    ...
                    $(callTree).click(function (event) {
                        ...
                        $(dest).dialog({
                            title: client.main.messages.label_select_df,
                            resizable: true,
                    ...
                    $(configWidget).find("input[name='endPointSource']").val($(divWidget).children("input[name='endPointSource']").val());
                    $(configWidget).find("input[name='endPointDecimalCulture']").val($(divWidget).children("input[name='endPointDecimalCulture']").val());
        ...
        buttons: {
            'Ok': function () {
                ...
                if (wdg_type == "text"
                    ...
                    $(divWidget).children("input[name='endPointSource']").val($(this).find("input[name='endPointSource']").val());
                    $(divWidget).children("input[name='endPointDecimalCulture']").val($(this).find("input[name='endPointDecimalCulture']").val());
                ...
                if (!ok) OnDashboardError(client.main.messages.label_allFieldsInserted);
                    ...
                    var wdg_text = [];
                    var wdg_title = clientParseJsonToObject($(divWidget).children("input[name='title']").val());
                    var wdg_content = clientParseJsonToObject($(divWidget).children("input[name='content']").val());
                    $.each(wdg_title.title, function (key, value) {

                        var obj = {
                            title: value,
                            content: (wdg_content != null) ? (wdg_content.hasOwnProperty('content')) ? wdg_content.content[key] : "" : "",
                            locale: key,
                        }
                        wdg_text.push(obj);
                    });
                    ...
                    // SAVE WIDGET
                    var id_row = $(divWidget).parents(".divRow").attr('id');
                    if (id_row != undefined) id_row = id_row.substring(4);

                    var id_wdg = $(divWidget).attr('id');
                    id_wdg = (id_wdg != undefined) ? id_wdg.substring(4) : -1;

                    data = {
                        id: id_wdg,
                        cssClass: $(divWidget).parent('.divTd ').attr('class'),
                        rowID: id_row,
                        cell: 0,
                        order: 1,
                        text: wdg_text,
                        type: wdg_type,
                        chartype: $(divWidget).children("input[name='chartType']").val(),
                        v: $(divWidget).children("input[name='v']").val(),
                        vt: $(divWidget).children("input[name='vt']").val(),
                        vc: $(divWidget).children("input[name='vc']").val(),
                        endPoint: $(divWidget).children("input[name='endPoint']").val(),
                        endPointType: $(divWidget).children("input[name='endPointType']").val(),
                        endPointV20: $(divWidget).children("input[name='endPointV20']").val(),
                        endPointSource: $(divWidget).children("input[name='endPointSource']").val(),
                        endPointDecimalSeparator: $(divWidget).children("input[name='endPointDecimalCulture']").val(),
                        dataflow_id: $(divWidget).children("input[name='id']").val(),
                        dataflow_agency_id: $(divWidget).children("input[name='agency']").val(),
                        dataflow_version: $(divWidget).children("input[name='version']").val(),
                        criteria: $(divWidget).children("input[name='criteria']").val(),
                        layout: $(divWidget).children("input[name='layout']").val(),
                        labels: $(divWidget).children("input[name='label_series']").val()
                    };
                    clientPostJSON(
                    (id_wdg==-1) ? _url.AddWidget : _url.UpdateWidget, clientParseObjectToJson(data),
                    function (jsonString) {

                        var wdg_dash = clientParseJsonToObject(jsonString);

                        $(divWidget).attr("id", 'wdg_' + wdg_dash.id);
                        $(divWidget).addClass("configured");
                        $(configWidget).dialog("close");
...
function AddDashboard(link) {
    ...
    if ($(".dashboardAdd").length == 0 && $(".divTable").length == 0) {
        ...
        str_html += "<input id='d_delete' type='button' value='" + client.main.messages.label_close + "' onclick='CloseDashboard(this)' />";
        str_html += "</div></div>";
        ...
        $("#d_continue").click(function () {
            ...
            if (til == "") {
                ...
                clientPostJSON(
                _url.CreateDashboard, clientParseObjectToJson(data),
                    function (jsonString) {

                        dashBoard = clientParseJsonToObject(jsonString);

                        if (dashBoard.hasOwnProperty('error')) return;

                        ReloadListDashboards("#dashboardsList");

                        // Create dashboard skeletron
                        DrawTable(dashBoard, dashBoard.id);
        ...
        OnDashboardError(client.main.messages.label_anotherDashboard);
    }
}
/*************************************/
function RemoveRow(link) {
    ...
    // if not if only row
    if ($('.divRow').length < 2) {
        OnDashboardError(client.main.messages.label_errorDeleteRow);
        return;
    }
...
function AddRow(link) {
    ...
    _url.AddRow, null,
        function (jsonString) {
            ...
            var inputDiv = "divSplit-" + row_dash.id;

            $('.divRow:last-child').after(
                '<div class="divRow" id="row_' + row_dash.id + '">' +
                    '<div class="setRow">' +
                        ...
                        '</select>'+
                    '</div>' +
                    '<div class="divSplit" id="' + inputDiv + '">' +
...
function RemoveWidget(div) {
    ...
    var divWidget = $(div).parent(".confWdg");
    var divContainer = $(divWidget).parents(".divTd");

    var id_widget = $(divWidget).attr('id');
    id_widget = (id_widget != undefined) ? id_widget.substring(4) : -1;
    ...
    ShowDialogDashboard(
        ...
        true, function () {
            clientPostJSON(
            _url.DeleteWidget, clientParseObjectToJson(data),
            function (jsonString) {
                $(divWidget).remove();
                $(divContainer).children(".selWidget").removeClass("hide");
            },
...
function Draw_Button_Filter(link) {
    ...
    if ($(divContainer).find("input[name='endPoint']").val() == "" ||
        ...
        var outCriteria = clientParseJsonToObject($(divContainer).find("input[name='criteria']").val());
        ...
        // se si vuole utilizzare modalita costraint
        clientPostJSON("Main/GetSpecificCodemap", clientParseObjectToJson(_data),
        function (jsonString) {
            ...
            if (jsonString.indexOf("CONFIG ERROR") > -1) {
                ...
                if (o.error == true) {
                    ...
                    var result = clientParseJsonToObject(jsonString);
                    DrawHTML_Filter(inputDataFlow, inputConfiguration, result.codelist_target, result.codemap, $(divContainer));
                }
            }
...
/*************************************/
function DrawHTML_Filter(df, kf, target,filters, sender) {
        ...
        function (args) {
            ...
            if ($(sender).find("input[name='widget_type']").val() == 'chart') {
                ...
                $.each(filters, function (idx, filter) {
                    ...
                    if (idx != undefined && idx != 'TIME_PERIOD') {
                        ...
                        if (!chartFilters.hasOwnProperty(idx)) {
                            if (filters.hasOwnProperty(idx) && chartFilters[idx]!=null)
                ...
                if (dimSelection > 1) {
                    ...
                    OnDashboardError(client.main.messages.label_dimoutlimit_desc);
...
function GetHTML_SettingsChart() {
    ...
    settingsChart += "<input type='hidden' name='endPointSource'/>";
    settingsChart += "<input type='hidden' name='endPointDecimalCulture'/>";
    ...
    settingsChart += '<option value="line">Line</option>';
    settingsChart += '<option value="area">Area</option>';
    settingsChart += '<option value="column">Column</option>';
    settingsChart += '<option value="stackedColumn">Stacked Column</option>';
    settingsChart += '<option value="pie">Pie</option>';
    settingsChart += '<option value="scatter">Scatter</option>';
    settingsChart += '</select>';
    settingsChart += '</div>';
    settingsChart += '<div class="clear-box"></div>';
...
function GetHTML_SettingsText() {
    ...
    settingsText += '<div class="area-full">';
    settingsText += '<textarea class="html_editor"></textarea>';
    settingsText += '</div>';
    settingsText += '</div>';
    return settingsText;
}
...
function GetHTML_SettingsTable() {
    ...
    settingsTable += '<input type="hidden" name="widget_type" value="table"/>';
    settingsTable += "<input type='hidden' name='endPointSource'/>";
    settingsTable += "<input type='hidden' name='endPointDecimalCulture'/>";
...
function GetHTML_ChartFields() {
    var chartFields = "<input type='hidden' name='widget_type' value='chart'/>" +
                        ...
                        "<input type='hidden' name='endPointSource'/>" +
                        "<input type='hidden' name='endPointDecimalCulture'/>" +
...
/*************************************/
function DrawTable(objDashBoard, dashBoardId) {

    var num = (objDashBoard.rows != null) ? objDashBoard.rows.length : 1;

    if (objDashBoard == undefined) return;
    ...
    for (i = 0; i < num; i++){
        ...
        var objRow = objDashBoard.rows[i];
        ...
        var objTitle = { wdgContentLang: objLang };

        var inputDiv = "divSplit-" + objRow.id;
        var widgets = "";

        if (objRow.widgets != null && objRow.widgets.length>0) {

            var countWidgets = objRow.widgets.length;
            ...
            $.each(objRow.widgets, function (k, v) {
                ...
                if (v.type == "text") {
                    ...
                    $(hiddenParams).find("input[name='endPointSource']").val(v.endPointSource);
                    $(hiddenParams).find("input[name='endPointDecimalCulture']").val(v.endPointDecimalSeparator);
                ...
                // ADD EXIST WIDGET
                widgets = widgets + '<div class="' + ((objRow.splitted) ? "divTd half" : "divTd full") + '">' +
                 "<div id='wdg_"+v.id+"' class='confWdg configured' >" +
                    "<input type='hidden' value='" + v.type + "' class='valConf'/>" +
            ...
            widgets = '<div class="' + ((objRow.splitted) ? "divTd half" : "divTd full") + '">' +
                            '<a class="selWidget" onclick="selectWidget(this);"><i class="icon-plus"></i>' + client.main.messages.label_addWidget + '</a>' +
                        '</div>' +
                        '<div class="' + ((objRow.splitted) ? "divTd half" : "divTd hide") + '">' +
                            '<a class="selWidget" onclick="selectWidget(this);"><i class="icon-plus"></i>' + client.main.messages.label_addWidget + '</a>' +
                        '</div>' +
                        '<div class="clear-box"></div>';
        }
        ...
        $(divRow).attr('id', 'row_' + objRow.id);
        $(divRow).addClass('divRow');
        var setRow = document.createElement('div');
        $(setRow).addClass('setRow');
        $(setRow).appendTo(divRow);

        var spanRemoveRow = document.createElement('span');
        $(spanRemoveRow).addClass('removeRow');
        $(spanRemoveRow).append('<a title="' + client.main.messages.label_removeRow + '" href="#" onclick="RemoveRow(this);"><i class="icon-cancel-squared"></i></a>');
        $(spanRemoveRow).appendTo(setRow);

        var selectChangeSlice = document.createElement('select');
        $(selectChangeSlice).addClass('changeSlice');
        $(selectChangeSlice).attr('onchange', "splitDiv('" + inputDiv + "',this);");
        $(selectChangeSlice).append('<option value="100%">100%</option><option ' + ((objRow.splitted) ? 'selected="selected"' : '') + ' value="50%">50%</option>');
        $(selectChangeSlice).appendTo(setRow);

        var divSplitter = document.createElement('div');
        $(divSplitter).attr('id', inputDiv);
        $(divSplitter).addClass('divSplit');
        $(divSplitter).append(widgets);
        $(divSplitter).append('<div class="clear-box"></div>');
        $(divSplitter).appendTo(divRow);
    ...
    $("#div_table").append('<div class="dashboardButtons">' +
                    //"<div class='dash-save'>" + " <input type='button' id='d-saveDash' value='" + client.main.messages.label_buttonSave + "' onclick='SaveDashboard(this," + dashBoard_ID + ");'/>" +"</div>" +
                    "<div class='dash-delete'>" +
                        " <input type='button' id='d-removeDash' value='" + client.main.messages.label_close + "' onclick='CloseDashboard(this," + dashBoard_ID + ");'/>" +
                    "</div>" +
                    "<div class='clear-box'></div>" +
                '</div>');
...
/*************************************/
function ShowDialogDashboard(title, text, useOk, callBackOk, useCancel, callBackCancel,clearContent) {
    ...
    $(div_fail).html((text != undefined) ? "<p>" + ((clearContent === false) ? "<span class='ui-icon ui-icon-circle-check' style='float:left; margin:0 7px 20px 0;'></span>" + text : text) + "</p>" : "");
...
function OnDashboardError(text) {
    ...
    $(div_fail).html("<p><span class='ui-icon ui-icon-alert' style='float:left; margin:0 7px 20px 0;'></span>" + text + "</p>");
    $(div_fail).dialog({
        resizable: false,
        position: { my: "center", at: "center", of: window },
        modal: true,
...
function clearOther(sender) {
    $(".sing_checkbox").prop('checked', false);
